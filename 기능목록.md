...
2. **DB 저장 방식**
   ...
   - `video_titles.product_data` JSON에 저장:
     - `deepLink`: 생성된 단축 링크
     - `productUrl`: 원본 Coupang URL (참고용)
   - 예시:
     ```json
     {
...
3. **수정 폼에서의 처리**
   - `startEdit()` 함수가 product_data의 `deepLink` 우선 추출
   - product_data에 deepLink가 있으면 그것을 사용
   - 없으면 product_data의 productUrl 사용
   - 그것도 없으면 DB의 product_url 컬럼 사용

#### Step 3: 내 목록(내목록)에 추가
1. **중복 상품 체크** ✅ (구현 완료)
   - 이미 내 목록에 있는 상품인지 먼저 확인
   - `product_url`로 중복 체크
   - 중복이면 스킵, 없으면 추가
   - 같은 상품을 여러 번 테스트해도 내 목록에는 한 번만 저장

2. **상품 저장 로직** ✅ (구현 완료)
   - `coupang_products` 테이블에 상품 INSERT (test-generate-stream에서 자동 실행)
   - 저장 항목:
     - `id` (자동 생성 ID)
     - `user_id` (사용자 ID)
...
1. **스케줄러가 신규 상품 감지** ✅ (구현 완료)
   - `checkAndRegisterCoupangProducts()` 함수가 coupang_products 테이블 모니터링
   - 3초 간격으로 신규 상품 감지
   - 구현 위치: `src/lib/automation-scheduler.ts` Line 1720

2. **자동 스케줄 생성** ✅ (구현 완료)
   - `video_titles` 테이블에 타이틀 생성
   - 상품 정보 저장 (product_data JSON)
   - `video_schedules` 테이블에 스케줄 자동 등록
   - 사용자의 활성 채널로 자동 배정
   - YouTube 프라이버시: 기본값으로 설정
