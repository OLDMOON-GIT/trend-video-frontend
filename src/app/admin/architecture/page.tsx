...
export default function ArchitecturePage() {
  ...
  const loadERD = async () => {
    try {
      ...
      if (!response.ok) {
        setErdError(data.error || 'ERD 문서를 불러올 수 없습니다.');
        return;
      }
      ...
      setErdError(err.message || 'ERD 문서 로딩 실패');
    }
  };
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">시스템 구조 및 데이터베이스 설계 문서</p>
          </div>
          <Link
            href="/admin"
...
{`graph TB
    ...
    style Storage fill:#0891b2,stroke:#22d3ee,color:#fff`}
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur p-8">
            {erdError ? (
              <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-8 text-center">
                ...
                <h2 className="text-2xl font-bold text-red-300 mb-2">문서 로드 실패</h2>
                <p className="text-slate-300">{erdError}</p>
              </div>
            ) : !erdMarkdown ? (
              <div className="text-center py-12">
                <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-purple-500 mx-auto mb-4"></div>
                <p className="text-slate-300 text-lg">ERD 문서 로딩 중...</p>
              </div>
            ) : (
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }: any) {
                      ...
                      if (!inline && language === 'mermaid') {
                        return (
                          <div
                            className="my-8 rounded-lg border border-purple-500/20 bg-slate-900/50 p-6 overflow-x-auto relative group cursor-pointer hover:border-purple-500/50 transition-all"
                            onClick={(e) => {
                              const svg = e.currentTarget.querySelector('svg');
                              if (svg) {
                                handleDiagramClick(svg as SVGElement);
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="relative max-h-[95vh] max-w-[98vw] overflow-hidden rounded-2xl border border-purple-500/30 bg-slate-900 p-6"
