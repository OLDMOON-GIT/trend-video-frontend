...
export async function GET(request: NextRequest) {
  ...
  try {
    // prompts 디렉토리에서 prompt_product_info.txt 파일 찾기
    const projectRoot = process.cwd();
    const promptsDir = path.join(projectRoot, 'prompts');
    const files = await fs.readdir(promptsDir);

    const promptFile = files.find(file => file === 'prompt_product_info.txt');

    if (!promptFile) {
      return NextResponse.json(
        { error: 'prompts/prompt_product_info.txt 파일을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
  ...
  <script>
    ...
    async function savePrompt() {
      ...
      try {
        ...
        const response = await fetch('/api/product-info-prompt', {
          method: 'PUT',
          headers: {
    ...
    console.error('Error reading product-info prompt file:', error);
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // 백업 디렉토리 생성
    const backupDir = path.join(projectRoot, 'backup', 'product-info-prompt-history');
    try {
    ...
    console.error('Error saving product-info prompt file:', error);
    return NextResponse.json(
