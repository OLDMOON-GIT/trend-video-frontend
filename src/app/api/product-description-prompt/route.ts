...
export async function GET(request: NextRequest) {
  ...
  try {
    // prompts 디렉토리에서 prompt_product_description으로 시작하는 .txt 파일 찾기
    const projectRoot = process.cwd();
    const promptsDir = path.join(projectRoot, 'prompts');
    const files = await fs.readdir(promptsDir);

    const promptFile = files.find(file =>
      file.startsWith('prompt_product_description') && file.endsWith('.txt')
    );

    if (!promptFile) {
      return NextResponse.json(
        { error: 'prompts/prompt_product_description으로 시작하는 .txt 파일을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
    ...
    if (promptCache &&
      ...
      console.log('📋 상품정보 프롬프트 캐시 사용:', promptFile);
      content = promptCache.content;
      cached = true;
    } else {
      console.log('📄 상품정보 프롬프트 파일 읽기:', promptFile);
...
<head>
  ...
  <title>상품정보 프롬프트 편집기 - ${promptFile}</title>
  <style>
...
<body>
  ...
  <div style="margin-bottom: 20px;">
    <a href="/" style="color: #f59e0b; text-decoration: none; font-weight: 600; font-size: 14px; transition: color 0.2s;" onmouseover="this.style.color='#fbbf24'" onmouseout="this.style.color='#f59e0b'">
      🏠 HOME
    </a>
    <span style="color: #64748b; margin: 0 8px;">/</span>
    <a href="/admin" style="color: #f59e0b; text-decoration: none; font-weight: 600; font-size: 14px; transition: color 0.2s;" onmouseover="this.style.color='#fbbf24'" onmouseout="this.style.color='#f59e0b'">
      관리자
    </a>
    <span style="color: #64748b; margin: 0 8px;">/</span>
    <a href="/admin/prompts" style="color: #f59e0b; text-decoration: none; font-weight: 600; font-size: 14px; transition: color 0.2s;" onmouseover="this.style.color='#fbbf24'" onmouseout="this.style.color='#f59e0b'">
      프롬프트 관리
    </a>
    <span style="color: #64748b; margin: 0 8px;">/</span>
    <span style="color: #94a3b8; font-weight: 600; font-size: 14px;">상품정보 프롬프트</span>
  </div>
  ...
  <script>
    ...
    async function savePrompt() {
      ...
      try {
        ...
        const response = await fetch('/api/product-description-prompt', {
          method: 'PUT',
          headers: {
    ...
    console.error('Error reading product description prompt file:', error);
    return NextResponse.json(
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // 백업 디렉토리 생성
    const backupDir = path.join(projectRoot, 'backup', 'product-description-prompt-history');
    try {
    ...
    try {
      ...
      const backupPath = path.join(backupDir, `prompt_product_description_${timestamp}.txt`);
      await fs.writeFile(backupPath, currentContent, 'utf-8');
      console.log('📦 이전 버전 백업:', `prompt_product_description_${timestamp}.txt`);
    } catch (error) {
    ...
    try {
      const multiAiPath = path.join(projectRoot, '..', 'trend-video-backend', 'src', 'ai_aggregator', 'prompt_product_description.txt');
      await fs.writeFile(multiAiPath, content, 'utf-8');
      console.log('✅ ai_aggregator 상품정보 프롬프트 동기화 완료');
    } catch (error) {
    ...
    console.log('✅ 상품정보 프롬프트 파일 저장 완료:', promptFile);

    return NextResponse.json({
      success: true,
      message: '상품정보 프롬프트가 성공적으로 저장되었습니다.',
      filename: promptFile
    });
  } catch (error) {
    console.error('Error saving product description prompt file:', error);
    return NextResponse.json(
