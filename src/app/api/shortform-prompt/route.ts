...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'restore' && body.version) {
      ...
      try {
        const multiAiPath = path.join(projectRoot, '..', 'multi-ai-aggregator', 'prompt_shortform.txt');
        await fs.writeFile(multiAiPath, backupContent, 'utf-8');
        console.log('✅ multi-ai-aggregator 숏폼 프롬프트 동기화 완료 (롤백)');
      } catch (error) {
    ...
    try {
      ...
      console.log('✅ multi-ai-aggregator 숏폼 프롬프트 동기화 완료');
    } catch (error) {
      console.warn('⚠️ multi-ai-aggregator 동기화 실패 (계속 진행):', error);
    }
