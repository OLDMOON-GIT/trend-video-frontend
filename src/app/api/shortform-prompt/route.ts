...
export async function GET(request: NextRequest) {
  ...
  try {
    // prompts 디렉토리에서 shortform_prompt로 시작하는 .txt 파일 찾기
    const projectRoot = process.cwd();
    const promptsDir = path.join(projectRoot, 'prompts');
    const files = await fs.readdir(promptsDir);

    const promptFile = files.find(file =>
      file.startsWith('shortform_prompt') && file.endsWith('.txt')
    );

    if (!promptFile) {
      return NextResponse.json(
        { error: 'prompts/shortform_prompt로 시작하는 .txt 파일을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }
    ...
    if (promptCache &&
      ...
      console.log('📋 숏폼 프롬프트 캐시 사용:', promptFile);
      content = promptCache.content;
      cached = true;
    } else {
      console.log('📄 숏폼 프롬프트 파일 읽기:', promptFile);
...
<head>
  ...
  <title>숏폼 프롬프트 편집기 - ${promptFile}</title>
  <style>
...
<body>
  ...
  <div style="margin-bottom: 20px;">
    ...
    <span style="color: #64748b; margin: 0 8px;">/</span>
    <span style="color: #94a3b8; font-weight: 600; font-size: 14px;">숏폼 프롬프트</span>
  </div>
  ...
  <script>
    ...
    async function savePrompt() {
      ...
      try {
        ...
        const response = await fetch('/api/shortform-prompt', {
          method: 'PUT',
          headers: {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'list-versions') {
      const backupDir = path.join(projectRoot, 'backup', 'shortform-prompt-history');
      ...
      try {
        ...
        const versions = backupFiles
          ...
          .map(f => {
            ...
            const match = f.match(/shortform_prompt_(.+?)(?:_before_restore)?\.txt/);
    ...
    if (action === 'restore' && body.version) {
      ...
      const currentBackupPath = path.join(backupDir, `shortform_prompt_${timestamp}_before_restore.txt`);
      ...
      try {
        const multiAiPath = path.join(projectRoot, '..', 'trend-video-backend', 'src', 'ai_aggregator', 'prompt_shortform.txt');
        await fs.writeFile(multiAiPath, backupContent, 'utf-8');
        console.log('✅ ai_aggregator 숏폼 프롬프트 동기화 완료 (롤백)');
      } catch (error) {
      ...
      console.log('✅ 숏폼 프롬프트 롤백 완료:', body.version);
    ...
    try {
      const currentContent = await fs.readFile(filePath, 'utf-8');
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
      const backupPath = path.join(backupDir, `shortform_prompt_${timestamp}.txt`);
      await fs.writeFile(backupPath, currentContent, 'utf-8');
      console.log('📦 이전 버전 백업:', `shortform_prompt_${timestamp}.txt`);
    } catch (error) {
    ...
    try {
      ...
      console.log('✅ ai_aggregator 숏폼 프롬프트 동기화 완료');
    } catch (error) {
      console.warn('⚠️ ai_aggregator 동기화 실패 (계속 진행):', error);
    }
    ...
    console.log('✅ 숏폼 프롬프트 파일 저장 완료:', promptFile);

    return NextResponse.json({
      success: true,
      message: '숏폼 프롬프트가 성공적으로 저장되었습니다.',
      filename: promptFile
    });
  } catch (error) {
