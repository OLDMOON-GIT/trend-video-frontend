...
**구현 위치**: `src/app/api/automation/test-generate-stream/route.ts` (Line 377-413)

**동작 방식**:
1. Step 2에서 딥링크 생성 완료 후 실행
2. **중복 체크**: 이미 내 목록에 있는 상품인지 확인 (product_url로 비교)
3. 중복이면 스킵, 없으면 `coupang_products` 테이블에 INSERT
4. 상품 정보 저장 (ID, 이름, 딥링크, 가격, 이미지)
5. status='active'로 설정하여 scheduler 감지 준비
...
`).run(
  ...
  user.userId,                 // 사용자 ID
  bestProduct.productUrl,      // Coupang 상품 URL (중복 체크용)
...
CREATE TABLE coupang_products (
  ...
  product_url TEXT NOT NULL,      -- 중복 체크용
  deep_link TEXT NOT NULL,        -- 딥링크
...
**중복 체크 특징**:
- `product_url`로 중복 체크 (같은 상품 URL은 한 번만 저장)
- 같은 상품을 여러 번 테스트해도 내 목록에는 한 번만 저장
- UI에서는 "⏸️ 이미 내 목록에 등록된 상품입니다 (스킵)" 메시지 표시
