...
// 딥링크 생성 테스트
async function testDeeplink(urls) {
  console.log(`\n🔗 딥링크 생성 테스트: ${urls.length}개 URL`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.rCode === '0' && data.data) {
        console.log(`   ✅ 성공: ${data.data.length}개 딥링크 생성`);
        data.data.forEach((item, index) => {
          if (item.shortenUrl) {
            console.log(`   ${index + 1}. ${item.shortenUrl}`);
          } else {
            console.log(`   ${index + 1}. ❌ 실패:`, item.message || 'Unknown error');
          }
        });
        return true;
      } else {
        console.log('   ❌ 실패:', data.message || '딥링크 생성 실패');
        return false;
      }
    } else {
...
(async () => {
  ...
  console.log('쿠팡 딥링크 생성 API 테스트');
  console.log('='.repeat(60));

  // 테스트용 쿠팡 상품 URL (실제 상품 URL로 교체 필요)
  const testUrls = [
    'https://www.coupang.com/vp/products/8366678245',
    'https://www.coupang.com/vp/products/1806346903'
  ];

  const result = await testDeeplink(testUrls);

  console.log('\n' + '='.repeat(60));
  console.log(`테스트 결과: ${result ? '✅ 통과' : '❌ 실패'}`);
  console.log('='.repeat(60));
